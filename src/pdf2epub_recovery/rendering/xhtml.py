        ...
        if element.element_type == "paragraph":
            if _is_bullet_text(element.text):
                if not list_open:
                    ...
                    list_open = True
                parts.append(
                    f'<li{_id_attr(element.element_id)}>'
                    f"{_render_inline_text(_strip_bullet_marker(element.text))}</li>"
                )
                continue
            ...
            parts.append(
                f'<p class="{css_class}"{_id_attr(element.element_id)}>'
                f"{_render_inline_text(element.text)}</p>"
            )
            ...
            parts.append(
                f'<figure class="image"{_id_attr(element.element_id)}>'
            ...
            if list_open:
                parts.append("</ul>")
                list_open = False
            parts.append(_render_callout(element.text, element.element_id))
...
def _is_bullet_text(text: str) -> bool:
    return text.lstrip().startswith(("\u2022", "â€¢"))
...
def _render_callout(text: str, element_id: str = "") -> str:
    parts = [part.strip() for part in text.split("\n\n") if part.strip()]
    if not parts:
        return f'<aside class="callout"{_id_attr(element_id)}></aside>'

    rendered = [f'<aside class="callout"{_id_attr(element_id)}>']
    first, *rest = parts
...
def _render_toc(element: DocumentElement) -> str:
    ...
    for entry in element.toc_entries:
        ...
        if entry.target_id:
            target = escape(f"#{entry.target_id}", quote=True)
            label = f'<a href="{target}">{entry_title}</a>'
        else:
            label = f"<span>{entry_title}</span>"
        parts.append(f'<li class="{css_class}">{label}{page_span}</li>')
    parts.append("</ol></nav>")
    return "".join(parts)
...
def _render_table(element: DocumentElement) -> str:
    if element.table is None:
        return _render_table_fallback(element.text, element.element_id)
    return _render_semantic_table(element.table, element.element_id)


def _render_table_fallback(text: str, element_id: str = "") -> str:
    rows = _pipe_rows(text)
    if rows:
        parts = [f'<figure class="table-fallback"{_id_attr(element_id)}><table>']
        header = rows[0]
        parts.append(
            "<thead><tr>"
    ...
    return (
        f'<figure class="table-fallback"{_id_attr(element_id)}>'
...
def _render_semantic_table(table: DocumentTable, element_id: str = "") -> str:
    rows = table.rows
    if not rows:
        return f'<figure class="table-fallback"{_id_attr(element_id)}><pre></pre></figure>'

    parts = [f'<figure class="table"{_id_attr(element_id)}><table>']
    body_rows = rows
    if table.header_row:
        header = rows[0]
        parts.append(
...
def _id_attr(element_id: str) -> str:
    if not element_id:
        return ""
    return f' id="{escape(element_id, quote=True)}"'
